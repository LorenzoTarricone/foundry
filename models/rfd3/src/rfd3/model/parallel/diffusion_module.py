...
from rfd3.model.parallel.layers.blocks import (
    local_token_transformer_cross_attn,
    compact_decoder_parallel,
    compact_decoder_parallel_sparse,
)
...
class ParallelDiffusionModule(RFD3DiffusionModule):
    ...
    def _diffusion_transformer_parallel(
        ...
        A_I_chunk = local_token_transformer_cross_attn(
            self.diffusion_transformer,                    # LocalTokenTransformer instance
    ...
    def _decoder_parallel(
        ...
        # Call decoder with cross-attention (free function from parallel/layers/blocks.py)
        A_I, Q_L, o = compact_decoder_parallel(
            self.decoder,                                  # CompactDecoder instance
            A_I=A_I,                                       # [B, I, c_token]
            S_I=S_I,                                       # [I, c_s]
            Q_L=Q_L,                                       # [B, L, c_atom]
            C_L=C_L,                                       # [B, L, c_atom]
            ...
            query_start=start_l,
            query_end=end_l,
            all_gather_fn=all_gather_along_dim,
            world_size=world_size,
        )
    ...
    def _decoder_parallel_sparse(
        ...
        # (free function from parallel/layers/blocks.py)
        A_I, Q_L, o = compact_decoder_parallel_sparse(
            self.decoder,                                  # CompactDecoder instance
            A_I=A_I,                                       # [B, I, c_token]
            S_I=S_I,                                       # [I, c_s]
            Q_L=Q_L,                                       # [B, L, c_atom]
