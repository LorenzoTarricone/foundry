...
def attention_pair_bias_forward_parallel(
    attn,  # AttentionPairBiasPairformerDeepspeed instance
    ...
    A_I_query_normed = attn.ln_1(A_I_query)
    A_I_key_normed = attn.ln_1(A_I_key)

    if attn.force_bfloat16:
    ...
    Q_chunk = attn.to_q(A_I_query_normed)
    G_chunk = attn.to_g(A_I_query_normed)
    K_all = attn.to_k(A_I_key_normed)
    V_all = attn.to_v(A_I_key_normed)

    # Pair bias from Z chunk
    B_chunk = attn.to_b(attn.ln_0(Z_chunk))
    ...
    Q_chunk = Q_chunk / torch.sqrt(
        torch.tensor(attn.c, device=Q_chunk.device, dtype=torch.bfloat16)
    )
