...
class RFD3DiffusionModule(nn.Module):
    ...
    def forward(
        ...
        parallel_mode=False,  # Flag from TokenInitializer
        **kwargs,
    ):
        """
        ...
        Args:
            ...
            parallel_mode: If True, Z_II is [I_par, I, c_z] (chunked tensor)
        ...
        # so encoder and other downstream code can access them
        if initializer_outputs is None:
            initializer_outputs = {}
        if is_parallel:
            # z_chunk_range should come from initializer_outputs or kwargs
            z_chunk_range = initializer_outputs.get("z_chunk_range") or kwargs.get("z_chunk_range")
            initializer_outputs["parallel_mode"] = True
            if z_chunk_range is not None:
    ...
    def forward_with_recycle(
        ...
        **kwargs,
        ...
        Args:
            ...
            parallel_mode: If True, Z_II is handled as [I_par, I, c_z] chunk
        ...
        **kwargs,
        ...
        with debug_time("DIFFUSION", "diffusion_token_encoder"):
            S_I, Z_II = self.diffusion_token_encoder(
                ...
                Z_init_II=Z_II,                                # [I, I, c_z]
                C_L=C_L,                                       # [B, L, c_atom]
                P_LL=P_LL,                                     # [L, L, c_atompair] or None
            )                                                  # Returns: [I, c_s], [I, I, c_z]
        ...
        with debug_time("DIFFUSION", "decoder"):
            if chunked_pairwise_embedder is not None:
                # Low-memory mode: sparse P_LL via chunked_pairwise_embedder
        ...
        return {
            "X_L": X_out_L,                                # [B, L, 3]
            "D_II_self": D_II_self,                        # [B, I, I, n_bins]
