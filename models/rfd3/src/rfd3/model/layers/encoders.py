...
from rfd3.model.debug_context import (
    debug_ctx, debug_log, debug_memory,
)
...
class TokenInitializer(nn.Module):
    ...
    For multi-GPU parallel mode, use ParallelTokenInitializer which
    overrides forward() to compute Z_chunk [I_par, I, c_z] per GPU.
    """
    ...
    def __init__(
        ...
        """Initialize token embedding module for RFD3.
        ...
        Args:
            ...
            relative_position_encoding: Config dict for RelativePositionEncodingWithIndexRemoval.
            n_pairformer_blocks: Number of Pairformer blocks in transformer_stack.
            pairformer_block: Config dict for PairformerBlock construction.
            downcast: Config dict for Downcast module (atom -> token pooling).
            token_1d_features: Config for token 1D feature embedding.
            atom_1d_features: Config for atom 1D feature embedding.
            atom_transformer: Config dict for LocalAtomTransformer (n_blocks, etc.).
            use_chunked_pll: If True, use sparse P_LL computation via ChunkedPairwiseEmbedder
                instead of materializing full [L, L, c_atompair] tensor.
        """
        super().__init__()
...
class DiffusionTokenEncoder(nn.Module):
    """Token-level diffusion encoder.

    Processes S_I through transitions, concatenates distogram/self-conditioning
    features with Z_II, then refines through a Pairformer stack.

    For multi-GPU parallel mode, use ParallelDiffusionTokenEncoder which
    overrides forward() to handle Z_chunk [I_par, I, c_z] per GPU.
    """
    ...
    def __init__(
        ...
        **_,
        ...
        """Initialize token-level diffusion encoder.

        Processes token features through transitions, distogram/self-conditioning
        concatenation, and a Pairformer stack.

        Args:
            c_s: Token single feature dimension.
            c_z: Token pair feature dimension.
            c_token: Token activation dimension (unused here, passed through).
            c_atompair: Atom pair feature dimension (unused here, passed through).
            sigma_data: Data normalization constant for distogram scaling.
            n_pairformer_blocks: Number of Pairformer blocks in pairformer_stack.
            pairformer_block: Config dict for PairformerBlock construction.
            use_distogram: If True, concatenate distogram features to Z_II.
            use_self: If True, concatenate self-conditioning distogram to Z_II.
            use_sinusoidal_distogram_embedder: If True, use SinusoidalDistEmbed
                for distogram (c_z output). If False, use bucketized distogram
                (n_bins_distogram=65 output).
        """
        super().__init__()
