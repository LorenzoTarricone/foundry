...
from rfd3.model.debug_context import debug_ctx, debug_log, debug_memory
...
class RFD3InferenceEngine(BaseInferenceEngine):
    ...
    def _run_multi(self, specs) -> None | Dict[str, List[RFD3Output]]:
        ...
        if debug_ctx.stats_enabled:
            debug_log("ENGINE", "_run_multi", f"rank={rank}, dist.is_initialized()={dist.is_initialized()}")

        debug_memory("ENGINE", "before_loader_setup")
        ...
        debug_memory("ENGINE", "after_loader_assembly")
        loader = self.trainer.fabric.setup_dataloaders(
            loader,
            use_distributed_sampler=False,
        )
        debug_memory("ENGINE", "after_fabric_setup")
        ...
        try:
            for batch_idx, batch in enumerate(loader):
                debug_memory("ENGINE", f"after_batch{batch_idx}_load")
            pipeline_output = batch[0]
            example_id = pipeline_output["example_id"]

            # Run model
            debug_memory("ENGINE", f"before_model_forward_{batch_idx}")
            ...
            debug_memory("ENGINE", "error_state")
            raise
        return outputs
